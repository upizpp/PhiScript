...
namespace phi {
    ...
    struct function_traits<ReturnType(Args...)> {
        ...
        using tuple_type = std::tuple<typename std::remove_cv<
            typename std::remove_reference<Args>::type>::type...>;
        using bare_tuple_type = std::tuple<typename std::remove_const<
            typename std::remove_reference<Args>::type>::type...>;
...
#define FUNCTION_TRAITS(...)                                                   \
    ...
    to_function(const Function &lambda) {
        return static_cast<
            typename function_traits<Function>::stl_function_type>(lambda);
    }
    ...
    template <size_t... Ts> struct index_seq {
        ...
        template <typename T> using append_t = typename append<T>::type;
    };

    template <size_t C, size_t E, size_t S = 1> struct gen_seq {
        using type = typename index_seq<C>::template append_t<
            typename gen_seq<C + S, E, S>::type>;
    };

    template <size_t E, size_t S> struct gen_seq<E, E, S> {
    ...
    template <size_t C> struct gen_index_seq {
        using type = typename gen_seq<0, C - 1, 1>::type;
    };

    template <> struct gen_index_seq<0> {
        using type = index_seq<>;
    };

    template <size_t C> using gen_index_seq_t = typename gen_index_seq<C>::type;

    template <typename T> struct FuncTraits;

    template <typename R, typename... A> struct FuncTraits<R(A...)> {
        using ret = R;
