...
const char *phiVersion() { return PHI_VERSION; }

Phi *phiCreate() { return new Phi; }

void phiDestroy(Phi *phi) { delete phi; }
...
PhiError *phiDoString(Phi *phi, const char *chunk) {
    ...
    try {
        ...
        phi->env =
            std::move(compiler.load().getMethod().call(phi->env, {}).second);
    } catch (const Exception &e) {
        return new PhiError{.msg = to_c_str(e.what()),
                            .type = to_c_str(e.className())};
    }
    return nullptr;
}

PhiError *phiDoStringWithName(Phi *phi, const char *chunk,
...
PhiError *phiDoFile(Phi *phi, const char *filename) {
    ...
    if (Pout::isPout(filename)) {
        phi->env = std::move(
            Pout::load(filename).getMethod().call(phi->env, {}).second);
        return nullptr;
    }
    try {
