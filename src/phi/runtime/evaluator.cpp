    ...
    Ref<Variant> Evaluator::eval() {
        ...
        for (_M_stream = 0; _M_stream < codes.size(); ++_M_stream,
            Singleton<ProgramFollower>::instance()->position(
                {"", _M_state->line(_M_stream), _M_state->chunk()})) {
...
#define UNARY_M_IMPL(opcode, method)                                           \
    ...
    Ref<Variant> Evaluator::handle(const OPCode &code) {
        ...
        case OPCode::Command::IMPORT: {
            ...
            Constant, and its value already exists in the -M_data variable
            */
        ...
        case OPCode::Command::CLOSURE_BIND: {
            ...
            if (func->type() != Variant::Type::FUNCTION)
                throw CompilerException{
                    "Bad OPCode::Command::CLOSURE_BIND. (Not a function)"};
        ...
        case OPCode::Command::CALL: {
            ...
            Singleton<ProgramFollower>::instance()->callBegin(
                {operand.nameSafely(), _M_state->line(_M_stream),
                 _M_state->chunk()});
        ...
        case OPCode::Command::LOAD_CONST: {
            push(Ref<Variant>{
                new Variant{_M_state->lookup(code.value())->deepCopy()}});
            break;
        }
    ...
    VariantPacker Evaluator::load(const string &name, bool throws) {
        ...
        if (throws)
            throw LoadException{
                "Attempt to load a unallocated variable. (\"%s\")",
                name.c_str()};
        else
            return {Variant::Null, name};
    }
    VariantPacker Environment::allocate(const string &name) {
        return {VariantPacker::variable_t{
                    &_M_locals.insert({name, new Variant}).first->second},
                name};
    }
    ...
    void Environment::free(const string &name) { _M_locals.erase(name); }
