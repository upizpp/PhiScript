...
namespace phi {
    ...
    class VariantPacker {
      ...
      public:
        ...
        VariantPacker(variable_t source)
            : _M_source(source), _M_data(*source) {}
        VariantPacker(Ref<Variant> data, const string &name)
            : _M_data(data), _M_name(new string{name}) {}
        VariantPacker(variable_t source, const string &name)
            : _M_source(source), _M_data(*source), _M_name(new string(name)) {}
        // for ARGS
        ...
        bool isArgs() const {
            return _M_source == nullptr && _M_data == nullptr;
        }
        ...
        VariantPacker &operator=(const VariantPacker &other) {
            return assign(other);
        }
        VariantPacker &operator=(const Ref<Variant> &other) {
            return assign(other);
        }
    ...
    class Evaluator {
      ...
      public:
        ...
        inline void popEnv() { _M_envs.pop_back(); }
      ...
      private:
        ...
        inline void clear() { _M_envs.clear(); }
        ...
        inline VariantPacker allocate(const string &name) {
            return get_env().allocate(name);
        }
