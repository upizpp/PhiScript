...
namespace phi {
    struct ProgramPosition {
        ...
        ProgramPosition(string f, uinteger l, const Ref<string> &c)
            : func(f), line(l), chunk(c) {}

        string toString() const {
            return "in \"" + *chunk + "\" at line " + std::to_string(line) +
                   (!func.empty() ? (": " + func) : "");
        }
    };

    inline std::ostream &operator<<(std::ostream &os,
                                    const ProgramPosition &position) {
    ...
    class ProgramFollower {
      ...
      public:
        ...
        void callEnd() { _M_calling.pop(); }
        const ProgramPosition &position() const { return _M_current; }
        void position(const ProgramPosition &position) {
            _M_current = position;
        }
        decltype(_M_calling) getCallStack() { return _M_calling; }
    };
} // namespace phi
