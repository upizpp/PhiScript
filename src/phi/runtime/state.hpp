...
namespace phi {
    class OPCodePacker {
      ...
      public:
        ...
        OPCodePacker(Borrower<CodeSeq> container, uinteger index)
            : _M_index(index), _M_container(container) {}
    ...
    class State {
      ...
      public:
        void print() const;
        const vector<arg_t> &lines() const { return _M_lines; }
        void lines(const vector<arg_t> &lines) { _M_lines = lines; }
        ...
        inline void chunk(Ref<string> chunk) { _M_chunk = chunk; }
        inline void setGCP(Ref<gcp_t> gcp) {
            _M_GCP = gcp;
            _M_globalPool = gcp;
        }
        const gcp_t &getGCP() const { return *_M_GCP; }
        inline void emitLabel(arg_t label) {
            emitLabel(label, static_cast<phi::arg_t>(_M_codes.size()));
        }
        inline void emitLabel(arg_t label, arg_t position) {
            _M_labels.insert({label, position});
        }
        ...
        inline Ref<Variant> lookup(arg_t id) const {
            return _M_GCP->find(id)->second;
        }
        ...
        inline const OPCode &getCode(uinteger index) const {
            return _M_codes[index];
        }
        inline const OPCode &top() const { return _M_codes.back(); }
        inline arg_t label(arg_t label) const { return _M_labels.at(label); }
        inline const unordered_map<arg_t, arg_t> &labels() const {
            return _M_labels;
        }
