...
namespace phi {
    namespace ast {
        ...
        State &Node::get_state() { return Generator::instance()->getState(); }

        OPCodePacker Node::push(const OPCode &code) {
            State &state = get_state();
            if (code.opt() == OPCode::Command::CLEAR &&
                state.top().opt() == OPCode::Command::CLEAR)
                return OPCodePacker();
        ...
        void Func::gen() {
            ...
            Function func = Compiler::load(
                _M_body, Compiler::globalOption ? *Compiler::globalOption
                                                : CompileOption{});
            func.getMethod().bind(_M_binds.release());
            Ref<Variant> f_variant = new Variant{func};
            f_variant->getPtr<Function>()->setThis(f_variant);
            push({OPCode::Command::LOAD_CONST,
                  Generator::instance()->push(f_variant)});
            if (_M_capture)
                for (auto &&value : *_M_capture) {
                    value.second->gen();
                    push({OPCode::Command::CLOSURE_BIND,
                          Generator::instance()->push(
                              new Variant{*value.first})});
                }
            if (_M_name)
        ...
        Eval::Eval(Ref<token::Token> tok, Ref<Block> expr)
            : Stmt(tok), _M_expr(expr), _M_block(Block::top) {
            if (!_M_block)
        ...
        void Load::gen() {
            push({_M_var ? OPCode::Command::ALLOCATE : OPCode::Command::LOAD,
                  Generator::instance()->push(
                      new Variant{((Ref<token::Word>)opt())->value()})});
        }
        ...
        void Constant::gen() {
            ...
            case token::Tag::FALSE:
                constant = new Variant{
                    ((Ref<token::Word>)data)->value() == "true" ? true : false};
                break;
            case token::Tag::INT:
            ...
            push({OPCode::Command::LOAD_CONST,
                  Generator::instance()->push(constant)});
        }

        void Eval::gen() {
            OPCodePacker *goto_code =
                new OPCodePacker{push({OPCode::Command::GOTO})};
        ...
        void Import::gen() {
            ...
            push({OPCode::Command::ALLOCATE,
                  Generator::instance()->push(
                      new Variant{_M_import_name->value()})});
            arg_t index = Generator::instance()->push(
                new Variant{_M_module_name->value()});
            push({OPCode::Command::IMPORT, index});
            push({OPCode::Command::ASSIGN});
        }

        Loop::loops_t Loop::_M_loops;
        void Loop::pushLoop(Borrower<Loop> loop) { _M_loops.push_back(loop); }
        ...
        Controller::Controller(Ref<token::Token> tok, const string &name)
            : Stmt(tok) {
            Borrower<Loop> loop = Loop::find(name);
            if (!loop)
                throw CompileException("Cannot find the loop(%s).",
                                       name.c_str());
            _M_loop = loop;
        }
    } // namespace ast
