        ...
        namespace ast
        {
            ...
            private:
                ...
                virtual bool _run() override
                {
                    ...
                    if (loop_stack.empty())
                        throw SyntaxException("Attempt to use break statement outside of loop.");
                    if (!found)
                        throw SyntaxException("Attempt to break out of a non-existent loop.");
                    while (!loop_stack.empty())
                    {
                        loop_stack.top()->breakLoop();
                        loop_stack.pop();
                    }
                    return false;
                }
            ...
            public:
                using Node::Node;
                virtual std::string nodeName() const override { return "BreakNode"; }
                virtual std::string stringify() const override
                {
                    std::ostringstream ss;
                    ss << Node::stringify() << '\t' << m_label;
                    return ss.str();
                }
