...
namespace phi {
    ...
    void Parser::match(token::tag_t tag) {
        if (_M_look->tag() == tag)
            ...
            throw SyntaxException(
                "Unexpected token '" + _M_look->stringify() + "' at line " +
                std::to_string(_M_look->line()) + ", expected '" +
                token::toString(tag) + '\'' + ".");
    }
    ...
    void Parser::locate() {
        if (_M_look)
            Singleton<ProgramFollower>::instance()->position(
                {"__compile__", _M_look->line(), _M_look->chunk()});
    }
    ...
    Parser::node_t Parser::body() {
        if (_M_look->tag() != '{') {
            ...
            res =
                new (res) Block{new Sequence{_M_look, expr_, nullptr}, _M_look};
    ...
    Parser::node_t Parser::blockOrReturn() {
        ...
        token_t tok = _M_look;
        node_t seq = new Sequence{
            _M_look,
            _M_look->tag() == Tag::RETURN ? expr() : new Return{tok, expr()},
            nullptr};
        auto eval_map = std::move(res->evalMap());
        res = new (res) Block{seq, tok};
        res->evalMap(std::move(eval_map));
    ...
    Parser::node_t Parser::commaLambda(bool required) {
        ...
        if (_M_look->tag() == Tag::ID)
            (binds = new vector<Ref<string>>)
                ->push_back(((Ref<Word>)_M_look)->valueP());
        node_t node = boolean();
        ...
        while (_M_look->tag() == ',') {
            ...
            x = (Node *)(new Comma{tok, x,
                                   new Comma{_M_look, boolean(), nullptr}});
        }
    ...
    Parser::node_t Parser::factor() {
        ...
        case '(':
            ...
            if (isLambda(x) && _M_look->tag() == '[') {
                ...
                Owner<vector<Ref<string>>> binds =
                    x && x->type() == Node::COMMA
                        ? std::move(((Ref<Comma>)x)->getIdentifiers())
                        : nullptr;
                return new Func{tok, nullptr, std::move(binds),
                                std::move(captured), blockOrReturn()};
            }
            if (_M_look->tag() == Tag::ARROW) {
                ...
                Owner<vector<Ref<string>>> binds =
                    x && x->type() == Node::COMMA
                        ? std::move(((Ref<Comma>)x)->getIdentifiers())
                        : nullptr;
                return new Func{tok, nullptr, std::move(binds), nullptr,
                                blockOrReturn()};
            }
            switch (_M_look->tag()) {
            case '(':
        ...
        case Tag::ID:
            ...
            case Tag::ARROW:
                ...
                return new Func{
                    tok, nullptr,
                    new vector<Ref<string>>{((Ref<Word>)tok)->valueP()},
                    nullptr, blockOrReturn()};
            default:
        ...
        case Tag::FN: {
            ...
            if (_M_look->tag() == Tag::ARROW)
                move();
            return new Func{tok, name, std::move(binds), std::move(captured),
                            blockOrReturn()};
        }
        case Tag::IMPORT: {
            tok = _M_look;
            ...
            case '[':
            case '.':
                return opt(new Import{tok, module_name, import_name});
            default:
        ...
        case Tag::IF: {
            ...
            if (_M_look->tag() == Tag::LIKELY ||
                _M_look->tag() == Tag::UNLIKELY)
                move();
            match('(');
            x = expr();
            match(')');
        ...
        case Tag::FOR: {
            ...
            Loop::pop();
            if (_M_look->tag() != Tag::ELSE)
                return &(new (node) For{tok, init, cond, update, body})
                            ->name(name);
            move();
            node_t else_body = this->body();
            return &(new (node)
                         ForElse{tok, init, cond, update, body, else_body})
                        ->name(name);
        }
        case Tag::BREAK: {
            tok = _M_look;
    ...
    Parser::node_t Parser::args() {
        ...
        while (_M_look->tag() == ',') {
            token_t tok = _M_look;
            move();
            x = (Node *)(new Args{tok, x,
                                  new Args{_M_look, boolean(), nullptr}});
        }
        return x;
    }
...
#define IS_OPT(tok)                                                            \
    ...
    Parser::node_t Parser::opt(node_t obj) {
        ...
        case '.': {
            ...
            match(Tag::ID);
            x = new Access{tok, obj,
                           new Args{symbol, new Constant(symbol), nullptr}};
            break;
        }
        case '[': {
            token_t tok = _M_look;
