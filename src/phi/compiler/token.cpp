...
#define REIDENTIFY_IMPL                                                        \
    ...
    return this
        ...
        Token *Token::reidentify() { REIDENTIFY_IMPL; }

        const Token *Token::reidentify() const { REIDENTIFY_IMPL; }

        bool Token::operator==(const Token &token) const {
            return equals(*this, token);
        }

        size_t Token::hash() const { return tag(); }

        size_t Integer::hash() const { return tag() + value(); }

        size_t Real::hash() const { return tag() + value(); }
